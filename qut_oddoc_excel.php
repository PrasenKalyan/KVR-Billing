...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='odbilling') or ($tsname=='naiduys') or ($tsname=='sumanthpotluri') or   ($tsname=='JFMOD22232823')){
											         $y="SELECT distinct quet_num,user FROM `odrequest_amnt` where  status='Amount Transferred' and bill_status='' or docr_status='Cancel'     ";
											    }else{
											         $y="SELECT distinct quet_num,user FROM `odrequest_amnt` where  status='Amount Transferred'  and bill_status=''   and user='$tsname'  or docr_status='Cancel'  ";
											    }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:R1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD DOCUMENT REQUIRED LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$ac=$row['ac_det'];
    $yt="select approve_amnt,ac_det,transfer_date from odrequest_amnt where quet_num='$qtno' and  confirm='Yes'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, $array);
	 $a10="select sum(approve_amnt) as req_amnt from odrequest_amnt where quet_num='$qtno' and  confirm='Yes' ";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper($row3['req_amnt'],'UTF-8'));
	 $a11="select transfer_date from odrequest_amnt where quet_num='$qtno' and  confirm='Yes' order by id desc limit 1 ";
...
header('Content-Disposition: attachment;filename="oddocument"'.$dt.'".xlsx"'); //tell browser what's the file name
