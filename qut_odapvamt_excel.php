...
if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='odbilling')){
$y="select * from odrequest_amnt where  confirm='Yes' and status='' order by id desc";    
}else{
$y="select * from odrequest_amnt where  confirm='Yes' and status='' and user='$tsname'  order by id desc";
}
...
$objPHPExcel->getActiveSheet()->getStyle("A4:X4")->getFont()->setBold(true)->getColor()->setRGB('ffffff');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD APPROVED AMOUNT LIST');
...
while($row	=	$result->fetch_assoc()){
    ...
    $krid=$row['id'];
    $ac=$row['ac_det'];
    $a="select store_code,ro_no,ro_date,falt_desc,tot_base,tot_ser,tot_gst,adv_amnt,adv_amnt1,adv_amnt2,gst_type,net,bal from add_odqot where quet_num='$qot'";
	...
	$adamt="select sum(approve_amnt) as req_amnt from odrequest_amnt where quet_num='$qot'and  status='Amount Transferred' and   confirm='Yes'" ;
												      $ads=$db->query($adamt) or die(mysqli_close($link));
												      $ads1=$ads->fetch_assoc();
												       $adsd=$ads1['req_amnt'];
	$objPHPExcel->getActiveSheet()->SetCellValue('S'.$rowCount, mb_strtoupper($adsd,'UTF-8'));
		$objPHPExcel->getActiveSheet()->SetCellValue('T'.$rowCount, mb_strtoupper($row['remarks'],'UTF-8'));
	
	$objPHPExcel->getActiveSheet()->SetCellValue('U'.$rowCount, mb_strtoupper($ac,'UTF-8'));
	$tyy="select sum(approve_amnt) as req_amnt from odrequest_amnt where quet_num='$qot'  and  confirm='Yes' and status=''   and ac_det='$ac'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('W'.$rowCount, mb_strtoupper(date('d.m.Y',strtotime($row['req_date'])),'UTF-8'));
...
$objPHPExcel->getActiveSheet()->SetCellValue('F'.$tc, 'Amount');
  ...
  if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
    	...
    	$y2="select distinct ac_det from odrequest_amnt where  user='$tsname' and  confirm='Yes' and status='' ";
	}
...
while($row10	=	$result10->fetch_assoc()){
    $acd=$row10['ac_det'];
	 ...
	 $r12="select sum(approve_amnt) as amt from odrequest_amnt where ac_det='$acd' and confirm='Yes' and status='' ";
