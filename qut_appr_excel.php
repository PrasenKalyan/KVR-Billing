...
$objPHPExcel->getActiveSheet()->mergeCells('A1:AH1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', $state.' PAYPEMENT RECEIVED LIST');
...
while($row	=	$result->fetch_assoc()){
    $q=$row['inv_no'];
	//	$st=$row['state'];
		$yu="select inv_date,inv_sub_date,speriod,ftype,sum(gst28) as gst28,sum(gst18) as gst18,sum(gst12) as gst12,sum(gst5) as gst5,sum(gst5) as gst5,sum(tbase) as tbase from ".$qotbill." where inv_num='$q'";
		$result1=$db->query($yu) or die(mysqli_close($link));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, mb_strtoupper($state,'UTF-8'));
...
header('Content-Disposition: attachment;filename="'.$state.'-paymentreceived.xlsx"'); //tell browser what's the file name
