...
$objPHPExcel->getActiveSheet()->mergeCells('A1:AF1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD QUOTATION LIST');
...
$objPHPExcel->getActiveSheet()->getStyle("A6:AF6")->getFont()->setBold(true)->getColor()->setRGB('ffffff');
  ...
  $y="SELECT * FROM add_odqot1    ORDER BY id desc";    
...
while($row10	=	$result->fetch_assoc()){
    $id1=$row10['id1'];
    $id2=$row10['id'];
    $y10="SELECT * FROM add_odqot where id='$id1'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper($row['ro_date'],'UTF-8'));
		...
		$ds1="select * from add_odqot1 where id1='$id1' and id='$id2'";
...
header('Content-Disposition: attachment;filename="odquotationsdetailslist.xlsx"'); //tell browser what's the file name
