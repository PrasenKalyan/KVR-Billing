...
if($qt_no1!=''){
	...
	$sq=mysqli_query($link,$a);
	 ...
	 if($qt_no1!=''){
	     ...
	     mysqli_query($link,"update qutcount set `count`='$cc' where state='OD'");
	$ac=mysqli_query($link,"update add_odqot set quet_num='$qt_no' where id='$sno'");
	 }
...
`desc1`='".addslashes($pname)."', `hsn`='$hsn', `gst`='$gst', `uom`='$uom', `qty`='$qty', `gst_amnt`='$gst_amnt',`remarks`='$remarks',
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
  ...
  $query = "INSERT INTO add_odqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,`remarks`) 
	VALUES 
...
$v="update `add_odbill1` set qr_code='$bar_code',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no' where id='$id'";
