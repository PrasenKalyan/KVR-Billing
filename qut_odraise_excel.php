...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
//$datatable="add_klqot";
 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='odbilling') or ($tsname=='Srujith.Nimmagadda') or ($tsname=='naiduys')){
 $y="SELECT * FROM odqot_bill where status='payment pending'   ";
	  }else{ 
 $y="SELECT * FROM odqot_bill where status='payment pending' and user='$tsadmin'  "; 
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:S1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD TO BE RAISE  QUOTATION LIST');
...
while($row	=	$result->fetch_assoc()){
    $q=$row['quet_num'];
   $k= "select * from odrequest_amnt where quet_num='$q'";
   $result1	=$db->query($k) or die(mysqli_close($link));
   $row1	=	$result1->fetch_assoc();
   $qtno=$row1['quet_num'];
  $u= "select * from add_odqot where quet_num='$qtno'";
...
header('Content-Disposition: attachment;filename="odraiseinvoice"'.$date.'".xlsx"'); //tell browser what's the file name
