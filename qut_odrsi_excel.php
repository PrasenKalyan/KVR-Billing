...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='odbilling')){
		 $y="select * from odqot_bill where status='RUn Paid' ";
		 }else{
		 $y="select * from odqot_bill where status='RUn Paid' and user='$tsname' ";
		}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD Raised Invoice LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper('OD','UTF-8'));
...
header('Content-Disposition: attachment;filename="odraisedinvoicelist.xlsx"'); //tell browser what's the file name
