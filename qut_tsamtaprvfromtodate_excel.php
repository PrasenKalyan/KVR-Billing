...
if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='knbilling') or ($tsname=='sumanthpotluri')){
	$y="select * from tgrequest_amnt where  confirm='Yes' and status='Amount Transferred' and date and date between '$fromdate' and '$todate'   order by id desc";    
}else{
...
$objPHPExcel->getActiveSheet()->getStyle("A4:AC4")->getFont()->setBold(true)->getColor()->setRGB('ffffff');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TELANGANA APPROVED AMOUNT LIST');
...
header('Content-Disposition: attachment;filename="tsaprvamountlist.xlsx"'); //tell browser what's the file name
