...
/*	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
	$y="select distinct quet_num,state from ".$request_amnt." where confirm='Pending' order by id desc";
    }else{
	$y="select distinct quet_num,state from ".$request_amnt." where confirm='Pending' and user='$tsname' order by id desc";
	}*/
	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='sumanthpotluri')){
     $y="select quet_num,state,user,gstamt,gsttype from ".$request_amnt." where confirm='Yes' and status='Amount Transferred' and date >= '$fromdate' and date <='$todate' order by id desc";
	}else{
	$y="select  quet_num,state,user,gstamt,gsttype from ".$request_amnt." where confirm='Yes' and status='Amount Transferred' and user='$tsname' and date >='$fromdate' and date<='$todate' order by id desc";
	}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:U1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', $state.'AP REQUESTED AMOUNT LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$tyy="select sum(req_amnt+gstamt) as rsamt from ".$request_amnt." where quet_num='$qot' and confirm='Yes'";
...
header('Content-Disposition: attachment;filename="'.$state.'-reqamountlist.xlsx"'); //tell browser what's the file name
