...
foreach($array as $value)
{
    ...
    $data=explode(":",$value);
       ...
       $id=$data[0];
				 ...
				 if($id!=''){
						$sq=mysqli_query($link,"select * from odrequest_amnt where id='$id'");
  ...
  <input type="hidden" name="ses" value="<?php echo $name;?>">
										...
										if($state=='OD'){
										 $a="select * from add_odqot where quet_num='$qot'";
												...
												$a="select * from add_odqot2 where quet_num='$qot'";
										}
									...
									<?php $idd=$r1['id'];?>
										...
										//	if($qot_type=='OD'){
											$sq=mysqli_query($link,"update add_odqot set req_amnt_settled='$amnt',status='Document Required' where quet_num='$qt_no'");
											
										//	} else {
										//		$sq=mysqli_query($link,"update add_odqot2 set req_amnt_settled='$amnt',status='Document Required' where quet_num='$qt_no'");
										//	}
											$ssq=mysqli_query($link,"update odrequest_amnt set transfer='$transfer',transfer_date='$transfer_date',status='Amount Transferred',remarks1='$remarks1' where id='$id'");
											
											print "<script>";
			print "alert('Amount Transferred Successfully');";
			print "self.location='odreq_list1.php';";
			print "</script>";
										}?>
											...
											&nbsp; &nbsp; &nbsp;
                                           <a href="odreq_list1.php"><button class="btn btn-danger" type="button" name="button" id="Close">
