...
$bid=$_GET['id'];
 $name1=$_GET['name1'];
  $name2=$_GET['name2'];
$loc=$_GET['loc'];
$q=mysqli_query($link,"select * from add_odqot where id='$bid'") or die(mysqli_error($link));
...
$r1=mysqli_fetch_array($ssq1);
 ...
 $superwisor=$r1['superwisor'];
 $coordinator=$r1['coordinator'];
 $sq1=mysqli_query($link,"SELECT * FROM `coordination` where name='$coordinator'");
 $r3=mysqli_fetch_array($sq1);
 $cor_email=$r3['email'];
  $sq11=mysqli_query($link,"SELECT * FROM `super` where name='$superwisor'");
 $r4=mysqli_fetch_array($sq11);
 $sup_email=$r4['email'];
...
?> 
 ...
 <table >
		...
		Vendor State Code :<br />
			...
			<td>
			<b> KVR BEST PROPERTY MANAGEMENT<br/>
			Road No:3, Sree Colony,Near Neredmet X Road, 
			Secunderabad - 500 056.<br/>
...
<td ><b>Quotation Number:</b><?php echo $qut=$r['quet_num'];?></tr>
...
<tr><td colspan="2"><b>Fault Description:</b><?php echo $r['falt_desc'];?></b></td></tr>
</table>
</td></tr>
...
<tr>
 ...
 ?> 
	...
	<tr>
	<th  colspan="14" style="text-align:right;padding-right:10px;font-size:20px;"> For Jyothi Facility Management<br/><img src="sign.jpeg" height="70"/><br/>Authorised Signatory
</th>
</tr>
...
$mpdf->Output('demo.pdf','F');
		...
		print "<script>";
	print "self.location='emailqotcheck.php?cord=$cor_email&supr=$sup_email&qot=$qut';";
	print "</script>";
