...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='odbilling')){
		 $y="select * from odqot_bill where status='Un Paid' ";
		 }else{
		 $y="select * from odqot_bill where status='Un Paid' and user='$tsname' ";
		}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD PAYPEMENT PENDING LIST');
...
header('Content-Disposition: attachment;filename="odpaymentpending.xlsx"'); //tell browser what's the file name
