...
$objPHPExcel->getActiveSheet()->mergeCells('A1:BJ1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'ODISHA TRACKER LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$ds10="select sum(approve_amnt) as apamount,sum(transfer) as kltft,sum(gstamt) as klgst from odrequest_amnt where quet_num='$qno'";
	$result20=$db->query($ds10) or die(mysqli_close($link));
	...
	$ds11="select transfer_date from odrequest_amnt where quet_num='$qno' ";
	$result21=$db->query($ds11) or die(mysqli_close($link));
	$row21=$result21->fetch_assoc();
	
	$objPHPExcel->getActiveSheet()->SetCellValue('AC'.$rowCount, mb_strtoupper(date('d-m-Y',strtotime($row21['transfer_date'])),'UTF-8'));
	$yt="select approve_amnt,ac_det,transfer_date from odrequest_amnt where quet_num='$qno'";
	...
	$ds1="select * from odqot_bill where quet_num='$qno'";
	$result11=$db->query($ds1) or die(mysqli_close($link));
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('AK'.$rowCount, mb_strtoupper('OD','UTF-8'));
	...
	$y25="select * from odpayment where inv_no='$invno'"; 
	$result12=$db->query($y25) or die(mysqli_close($link));
...
header('Content-Disposition: attachment;filename="odtrack.xlsx"'); //tell browser what's the file name
