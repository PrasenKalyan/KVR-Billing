...
/*	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
	$y="select distinct quet_num,state from odrequest_amnt where confirm='Pending' order by id desc";
    }else{
	$y="select distinct quet_num,state from odrequest_amnt where confirm='Pending' and user='$tsname' order by id desc";
	}*/
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:U1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'OD REQUEST AMOUNT LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$tyy="select sum(req_amnt+gstamt) as rsamt from odrequest_amnt where quet_num='$qot' and confirm='Pending'";
	...
	$yt1="select gsttype,gstamt from odrequest_amnt where quet_num='$qot'";
...
$objPHPExcel->getActiveSheet()->SetCellValue('R'.$rowCount, (rtrim($array1,',')));
$objPHPExcel->getActiveSheet()->SetCellValue('S'.$rowCount, (rtrim($array2,',')));
	
	
	
	
	
	$yt="select req_amnt,gstamt,ac_det,req_date from odrequest_amnt where quet_num='$qot'";
...
header('Content-Disposition: attachment;filename="odreqamountlist.xlsx"'); //tell browser what's the file name
